...
RUN \
  ...
  if [ -n "${CURSOR_VERSION}" ]; then \
    ...
    CURSOR_API_RESPONSE=$(curl -s 'https://www.cursor.com/api/download?platform=linux-arm64&releaseTrack=stable') && \
  ...
  dpkg -i ghdesktop.deb && \
  apt-get install -f -y && \
  \
  echo "**** Adding Cursor icon ****" && \
  curl -o /kclient/public/icon.png https://www.cursor.com/apple-touch-icon.png && \
  \
  echo "**** Cleanup ****" && \
